...
class websocket{
	...
	function uncode($str){
		...
		function eventoutput($type,$event){//事件回调
			...
			$method = $class->getmethod($this->event);
			$method->invokeArgs($instance, [$type,$event, $this]);
			// $res = $method->invoke($instance);
			// var_dump($res,$this->event,$instance);die;
			// // call_user_func([$this->obj,$this->event],$type,$event);
		}
