...
class  ThinkTemplate {
    ...
    public function parse($content) {
        ...
        $content = preg_replace_callback('/'.$begin.'literal'.$end.'(.*?)'.$begin.'\/literal'.$end.'/is', function($match){return $this->parseLiteral($match[1]);}, $content);
    ...
    protected function parseExtend($content) {
        ...
        if($find) {
            ...
            $content = preg_replace_callback('/'.$begin.'block\sname=(.+?)\s*?'.$end.'(.*?)'.$begin.'\/block'.$end.'/is', function($match){return $this->parseBlock($match[1],$match[2]);}, $content);
            // 读取继承模板
            $array      =   $this->parseXmlAttrs($matches[1]);
